...
if [[ "$(type -P rbenv)" ]]; then
  ...
  if [[ "$list" ]]; then
    ...
    for version in $list; do unsudo rbenv install "$version"; done
    [[ "$(echo "$list" | grep -w "${versions[0]}")" ]] && unsudo rbenv global "${versions[0]}"
    unsudo rbenv rehash
  fi
fi
